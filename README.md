...
The `slurm-*.sbatch` launchers are generated from `templates/*.sbatch.tpl`:

```bash
python tool-generate-slurm.py                 # all stages, with the default values
python tool-generate-slurm.py download --workers 20
python tool-generate-slurm.py processing --cpus 40 --workers 32 --account foo --partition cpu_long --time 2-00:00:00
```
...
Run this pipeline by launching two parallel tasks, one running the `slurm-processing.sbatch` (Check the model used) and one the `slurm-download.sbatch` (Needs a list of IIIF manifests).
...
Utilities: `stats-count-pages-from-archives.py` (page counts), `bugfix-check-undownloaded.py` (finds missing images), `tool-rebuild-targz-index.py` (backfills `targz/index.csv` for archives made by older pipeline versions — idempotent, run once after upgrading).
